class ContaBancaria {
  ...
  depositar(valor) {
    ...
    if (isNaN(cents) || cents <= 0)
      return { ok: false, message: "Valor inválido para depósito." };
    if (cents > 100000 * 100)
      return {
        ok: false,
    ...
    return {
      ok: true,
      message: `Depósito de ${this.formatCents(cents)} realizado com sucesso!`,
    };
  }
  ...
  sacar(valor) {
    const cents = this.parseToCents(valor);
    if (isNaN(cents) || cents <= 0)
      return { ok: false, message: "Valor inválido para saque." };
    if (cents > this.saldoCents)
    ...
    return {
      ok: true,
      message: `Saque de ${this.formatCents(cents)} realizado com sucesso!`,
    };
  }

  getSaldoFormatado() {
    return this.formatCents(this.saldoCents);
  }
...
function mostrarMensagem(msg, destaque = false) {
  resultadoDiv.innerHTML =
    (destaque ? `<strong>${msg}</strong>` : msg) +
...
function atualizarHistorico() {
  ...
  txs.forEach((t) => {
    ...
    li.innerHTML = `<div><strong>${t.type}</strong> — ${conta.formatCents(
      t.amountCents
    )}</div>
...
document.getElementById("sair").addEventListener("click", () => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.location.href = "/indexLanpage.html";
    }
  });
});
